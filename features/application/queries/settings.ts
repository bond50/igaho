...
export async function getApplicationPortalSetting() {
    try {
        return await db.applicationPortalSetting.findUnique({
            where: {singletonKey: 'default'},
        });
    } catch (error) {
        console.warn('getApplicationPortalSetting fallback (DB/table not ready yet):', error);
        return null;
    }
}
...
export async function getPortalBranding() {
    try {
        ...
        console.warn('getPortalBranding fallback (DB/table not ready yet):', error);
        return {
